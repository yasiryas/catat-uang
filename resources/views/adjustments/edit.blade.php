        ...
        <form method="POST" action="{{ route('adjustments.update', $adjustment) }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="period_id" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" required>
                    ...
                    @foreach ($periods as $p)
                        <option value="{{ $p->id }}" {{ old('period_id', $adjustment->period_id) == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                    @endforeach
                </select>
                @error('period_id')
            ...
            <div>
                ...
                <select name="type" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" required>
                    <option value="income" {{ old('type', $adjustment->type) === 'income' ? 'selected' : '' }}>Pemasukan</option>
                    <option value="expense" {{ old('type', $adjustment->type) === 'expense' ? 'selected' : '' }}>Pengeluaran</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="date" name="date" value="{{ old('date', $adjustment->date) }}" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" required>
                @error('date')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="note" rows="3" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" placeholder="Catatan...">{{ old('note', $adjustment->note) }}</textarea>
                @error('note')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
