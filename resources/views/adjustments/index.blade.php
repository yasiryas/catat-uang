...
@section('content')
    ...
    @endphp
    <div x-data="adjustmentPage()" data-periods='{{ $periodsJson }}'>
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold">Adjustment</h1>
                <p class="text-slate-500">Kelola penyesuaian pemasukan dan pengeluaran.</p>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-200">
                        @forelse($adjustments as $adjustment)
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="p-4 text-right font-medium">Rp {{ number_format((float) $adjustment->amount, 0, ',', '.') }}</td>
                                <td class="p-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <button @click="edit(@json($adjustment))" class="text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors">Edit</button>
                                        <button @click="confirmDelete({{ $adjustment->id }})" class="text-red-600 hover:text-red-800 font-medium text-sm transition-colors">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="p-10 text-center text-slate-500">Belum ada adjustment.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="modal.show" style="display: none" class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50 flex items-center justify-center">
            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-xl font-bold" x-text="modal.mode === 'create' ? 'Tambah Adjustment' : 'Edit Adjustment'"></h2>
        ...
        <div x-show="deleteModal.show" style="display: none" class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50 flex items-center justify-center">
            ...
            <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-semibold mb-2">Hapus Adjustment</h3>
                <p class="text-slate-600 mb-6">Yakin ingin menghapus adjustment ini?</p>
