    ...
    <div x-data="accountPage()">
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-slate-500">Kelola dompet / lokasi uang Anda.</p>
            </div>
                ...
                <table class="w-full">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th class="text-left p-4 font-medium text-slate-700">Tipe</th>
                    ...
                    <tbody class="divide-y divide-slate-200">
                        @forelse($accounts as $account)
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="p-4">
                                    @php
                                        $typeLabels = ['bank' => 'Bank', 'cash' => 'Tunai', 'ewallet' => 'E-Wallet'];
                                        $typeColors = ['bank' => 'bg-blue-100 text-blue-800', 'cash' => 'bg-emerald-100 text-emerald-800', 'ewallet' => 'bg-purple-100 text-purple-800'];
                                    @endphp
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $typeColors[$account->type] ?? 'bg-slate-100 text-slate-800' }}">
                                        {{ $typeLabels[$account->type] ?? $account->type }}
                                    </span>
                                </td>
                                <td class="p-4 text-slate-500">{{ $account->description ?? '-' }}</td>
                                <td class="p-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <button @click="edit({{ $account->id }}, '{{ $account->name }}', '{{ $account->type }}', '{{ $account->description }}')" class="text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors">Edit</button>
                                        <button @click="confirmDelete({{ $account->id }}, '{{ $account->name }}')" class="text-red-600 hover:text-red-800 font-medium text-sm transition-colors">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="p-10 text-center text-slate-500">Belum ada dompet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="modal.show" style="display: none" class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50 flex items-center justify-center">
            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-xl font-bold" x-text="modal.mode === 'create' ? 'Tambah Dompet' : 'Edit Dompet'"></h2>
                ...
                <form @submit.prevent="submit()">
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" x-model="modal.form.name" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" placeholder="Contoh: Mandiri, BCA, Cash" required>
                        ...
                        <div>
                            ...
                            <textarea x-model="modal.form.description" rows="3" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" placeholder="Keterangan tambahan..."></textarea>
                        </div>
                    </div>
        ...
        <div x-show="deleteModal.show" style="display: none" class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50 flex items-center justify-center">
            ...
            <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-semibold mb-2">Hapus Dompet</h3>
                <p class="text-slate-600 mb-6">Yakin ingin menghapus <strong x-text="deleteModal.account?.name"></strong>?</p>
