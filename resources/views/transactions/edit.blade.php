    ...
    <div class="max-w-3xl mx-auto bg-white rounded-xl shadow p-6">
        ...
        <form method="POST" action="{{ route('transactions.update', $transaction) }}" class="space-y-4">
            ...
            <div>
                ...
                <x-custom-select id="period_id" name="period_id" variant="input"
                    placeholder="Pilih Periode"
                    :items="$periods->map(fn($p) => ['id' => $p->id, 'name' => $p->name])->toArray()"
                    value-key="id" label-key="name" value="{{ old('period_id', $transaction->period_id) }}" />
            ...
            <div>
                ...
                <x-custom-select id="category_id" name="category_id" variant="input"
                    ...
                    value-key="id" label-key="name" value="{{ old('category_id', $transaction->category_id) }}" />
